			...
			: ($action="export@")
				...
				Case of 
					...
					: ($action="exportHTML")
						...
						//$options[‘k81;369‘‘k81;369‘]:=Vrai  // new 19R6 (TRUE by default)
					...
					: ($action="exportDocx")  // WORD
						...
						$options:=New object:C1471
						//$options[‘k81;369‘‘k81;369‘]:=Faux  // new 19R6 (false by default)
						$options.extra:=New object:C1471
				...
				If (ok=1)
					...
					If (ok=1) & ($docName#"")
						...
						If ($action="exportSVG")
							...
							If (Test path name:C476($folderPath)=Est un dossier:K24:2)
								DELETE FOLDER:C693($folderPath; Supprimer avec contenu:K24:24)
							End if 
							CREATE FOLDER:C475($folderPath)  // if path exists, finder manages and request 
							
							If (Substring:C12($folderPath; Length:C16($folderPath); 1)#Séparateur dossier:K24:12)
								$folderPath:=$folderPath+Séparateur dossier:K24:12
							End if 
